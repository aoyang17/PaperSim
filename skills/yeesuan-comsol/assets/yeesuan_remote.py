...
DEFAULT_CONFIG = Path("~/.config/papersim/comsol_remote.json").expanduser()
DEFAULT_PASSWORD = Path("~/.config/papersim/comsol_password").expanduser()


def _load_papersim():
    source = Path(os.environ.get("PAPERSIM_SRC") or Path(__file__).resolve().parents[3] / "src").expanduser()
    ...
    try:
        ...
        raise SystemExit(
            "Cannot import PaperSim COMSOL transport. Set PAPERSIM_SRC or install papersim."
        ) from exc
    return ComsolConfig, ComsolRemoteAgent
...
def _check(args: argparse.Namespace) -> int:
    ...
    result = {
        ...
        "password_mode": _mode(password_path),
        ...
        "auth_mode": "password+publickey" if identity else "password",
...
def _agent(args: argparse.Namespace):
    ComsolConfig, ComsolRemoteAgent = _load_papersim()
    return ComsolRemoteAgent(
        ComsolConfig.load(args.config),
...
def _remote_command(args: argparse.Namespace) -> int:
    ...
    if args.action == "mkdir":
        ...
        command = (
            f"source {_remote_path(loaded.environment_script)} && "
            f"squeue -j {job} -o '%.18i %.9P %.24j %.8T %.10M %.6D %R'; "
