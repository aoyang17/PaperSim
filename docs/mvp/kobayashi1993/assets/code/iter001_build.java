 ...
 */
public final class Kobayashi1993DendriteBuild {
  ...
  private static void parameters() {
    parameter("L", "9[m]", "square domain side");
    parameter("epsbar", "0.01[m]", "mean interface width");
    parameter("tau", "0.0003[s]", "phase relaxation time");
    parameter("alpha", "0.9", "liquidus slope near equilibrium");
    parameter("gamma", "10.0", "inverse supercooling scale");
    parameter("Teq", "1.0", "equilibrium temperature");
    parameter("jmode", "4", "fourfold anisotropy mode");
    parameter("delta", "0.02", "anisotropy strength swept in Fig. 7");
    parameter("theta0", "0[rad]", "anisotropy axis");
    parameter("Klatent", "2.0", "dimensionless latent heat for Fig. 7");
    parameter("D", "1.0", "dimensionless thermal diffusivity set to unity");
    parameter("noiseAmp", "0.01", "uniform noise amplitude");
    parameter("noiseSeed", "1993", "fixed random-sequence seed");
    parameter("hnoise", "0.03[m]", "noise correlation cell");
    parameter("dtnoise", "0.0002[s]", "noise update cadence");
    parameter("R0", "0.15[m]", "initial circular seed radius");
    parameter("hmesh", "0.03[m]", "mapped mesh element size");
    parameter("maxStep", "0.0002[s]", "maximum BDF time step");
    parameter("dtout", "0.01[s]", "stored output interval");
    parameter("tfinal", "1.4[s]", "Fig. 7 requested final time");
  }
  ...
  private static void variables() {
    ...
    // v_geometry
    variable("v_geometry", "xCenter", "L/2", "x coordinate of the square-domain centre");
    variable("v_geometry", "yCenter", "0", "y coordinate of the square-domain centre");
    variable("v_geometry", "radius", "sqrt((x-xCenter)^2+(y-yCenter)^2)", "radial distance from the seed centre");

    // v_interface
    variable("v_interface", "theta", "atan2(-py,-px)", "interface-normal angle");
    variable("v_interface", "anisArg", "jmode*(theta-theta0)", "anisotropy angular argument");
    variable("v_interface", "epsilon", "epsbar*(1+delta*cos(anisArg))", "anisotropic interface thickness");
    variable("v_interface", "epsilonTheta", "-epsbar*delta*jmode*sin(anisArg)", "angular derivative of interface thickness");

    // v_source
    variable("v_source", "mT", "alpha/pi*atan(gamma*(Teq-T))", "thermodynamic driving force");
    variable("v_source", "noiseHashArg", "12.9898*floor(x/hnoise)+78.233*floor(y/hnoise)+37.719*floor(t/dtnoise)+noiseSeed", "deterministic hash-noise phase argument");
    variable("v_source", "chi", "mod(abs(sin(noiseHashArg)*43758.5453),1)-0.5", "deterministic uniform noise field");

    // v_flux
    variable("v_flux", "heatFluxX", "-D*Tx", "x component of named thermal flux");
    variable("v_flux", "heatFluxY", "-D*Ty", "y component of named thermal flux");
    variable("v_flux", "GammaPx", "nojac(epsilon*epsilonTheta)*py-nojac(epsilon^2)*px", "x component of anisotropic phase-field flux");
    variable("v_flux", "GammaPy", "-nojac(epsilon*epsilonTheta)*px-nojac(epsilon^2)*py", "y component of anisotropic phase-field flux");

    // v_source
    variable("v_source", "latentSource", "Klatent*d(p,t)", "latent-heat source for the temperature balance");
    variable("v_source", "phaseSource", "p*(1-p)*(p-0.5+mT)+noiseAmp*p*(1-p)*chi", "named phase-field source including deterministic noise");

    // v_initial
    variable("v_initial", "phaseInitial", "0.5*(1-tanh((radius-R0)/(sqrt(2)*epsbar)))", "smooth circular initial phase seed");
    variable("v_initial", "temperatureInitial", "0", "uniform initial supercooled temperature");

    // v_diagnostics
    variable("v_diagnostics", "solidArea", "intop1(p)", "post-processing solid area proxy");
    variable("v_diagnostics", "enthalpyInvariant", "intop1(T-Klatent*p)", "post-processing enthalpy invariant");
    variable("v_diagnostics", "tipY", "maxop1(if(p>=0.5,y,0))", "maximum vertical solid coordinate");
    variable("v_diagnostics", "halfWidth", "maxop1(if(p>=0.5,abs(x-xCenter),0))", "maximum horizontal solid coordinate");
    variable("v_diagnostics", "pMin", "minop1(p)", "minimum phase-field value");
    variable("v_diagnostics", "pMax", "maxop1(p)", "maximum phase-field value");
    variable("v_diagnostics", "TMin", "minop1(T)", "minimum dimensionless temperature");
    variable("v_diagnostics", "TMax", "maxop1(T)", "maximum dimensionless temperature");
  }
  ...
  private static void study() {
    ...
    m.study("std1").feature("param").set("plistarr", new String[]{"0,0.005,0.01,0.02,0.05"});
  ...
  public static void main(String[] args) throws Exception {
    m = ModelUtil.create("Kobayashi1993Dendrite");
    m.label("kobayashi1993_dendrite iter001 build-only");
